...
async function syncStores(items) {
	...
	for(let key of Object.keys(localStorage).filter(key => key.startsWith('curvebtc_'))) {
		...
		await fetch('https://pushservice.stablexswap.com/db/upserttx', {
			method: 'POST',
			headers: {
...
export async function useFirestore() {
	...
	try {
		let user = await firebaseApp.auth().createUserWithEmailAndPassword(`${contract.default_account}@stablexswap.com`, password)
	}
	catch(err) {
		console.error(err)
		console.log("SIGN IN INSTEAD", password)
		try {
			let user = await firebaseApp.auth().signInWithEmailAndPassword(`${contract.default_account}@stablexswap.com`, password)
		}
		catch(err) {
			console.error(err)
		}
		...
		// 	let user = firebaseApp.auth().signInWithEmailAndPassword(`${contract.default_account}@stablexswap.com`, password)
		// }
	}
...
export async function getAllItems() {
	...
	if(state.fireUser) {
		...
		try {
			let dbdata = await fetch('https://pushservice.stablexswap.com/db/transactions', {
				method: 'POST',
				headers: {
					'Content-type': 'application/json',
				},
