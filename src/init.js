...
// Check that this works to send notifications to BSCScan link
export function notifyHandler(hash) {
  let { emitter } = notify.hash(hash)
  emitter.on('all', transaction => ({
      onclick: () => window.open(`https://bscscan.com/tx/${transaction.hash}`, '_blank', 'noopener, norefferer')
    })
  )
}
...
// Updated compatible wallets list
let wallets = [
  { walletName: "metamask" },
  // {
  //   walletName: "trezor",
...
if(isMobile && !window.web3) {
  wallets = wallets.filter(wallet => !['trust', 'imToken'].includes(wallet.walletName))
}
